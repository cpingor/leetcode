...
# 
# 你可以随时停下摩天轮，即便是 在服务所有游客之前 。如果你决定停止运营摩天轮，为了保证所有游客安全着陆，将免费进行所有后续轮转,注意，如果有超过4
...
class Solution:
    def minOperationsMaxProfit(self, customers: List[int], boardingCost: int, runningCost: int) -> int:
        ...
        for customer in customers:
            customers_rest += customer
            if customers_rest > 4:
                revenue += boardingCost * 4 - runningCost
                customers_rest -= 4
            else:
                revenue += boardingCost * customers_rest - runningCost
                customers_rest = 0
            i += 1
            result.append([revenue, i])
        j = customers_rest // 4
        customers_last = customers_rest - 4 * j
        if customers_last * boardingCost <= runningCost:
            customers_rest = 4 * j
            revenue += boardingCost * customers_rest - runningCost * j
            result.append([revenue, i + j])
        else:
            j += 1
            revenue += boardingCost * customers_rest - runningCost * j
            result.append([revenue, i + j])
        ...
        for item in result:
            if item[0] > max_res:
                max_res = item[0]
                temp_res = item[1]
        return temp_res if max_res > 0 else -1
